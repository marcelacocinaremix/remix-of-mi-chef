-- Add daily usage tracking columns to user_subscriptions
ALTER TABLE public.user_subscriptions 
ADD COLUMN IF NOT EXISTS daily_uses integer NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS last_use_date date;

-- Create function to check and increment daily uses
CREATE OR REPLACE FUNCTION public.check_and_increment_daily_uses(p_user_id uuid, p_daily_limit integer DEFAULT 10)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    result := jsonb_build_object(
      ...
      'uses_today', current_uses,
      'remaining', 0,
      'message', '¡Llegaste al límite de recetas por hoy! Volvé mañana para seguir cocinando 🍳'
    );
  END IF;
