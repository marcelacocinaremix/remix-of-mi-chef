...
BEGIN
  ...
  -- Allow: the user themselves OR service role (auth.uid() is NULL when called with service role key)
  IF calling_uid IS NOT NULL AND calling_uid != p_user_id THEN
    RAISE EXCEPTION 'Unauthorized: can only modify own usage';
  END IF;
