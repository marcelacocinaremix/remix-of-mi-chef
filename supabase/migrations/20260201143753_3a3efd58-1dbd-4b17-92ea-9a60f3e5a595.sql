-- Update daily uses function to use a 4-recipes daily limit by default and return a dynamic message
CREATE OR REPLACE FUNCTION public.check_and_increment_daily_uses(p_user_id uuid, p_daily_limit integer DEFAULT 4)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    result := jsonb_build_object(
      ...
      'uses_today', current_uses,
      'remaining', 0,
      'message', format('¡Llegaste al límite de %s recetas por hoy! Volvé mañana 🍳', p_daily_limit)
    );
  END IF;
