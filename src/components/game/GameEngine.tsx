...
function getRecipeOptions(correctRecipeId: string): typeof GAME_RECIPES {
...
export function GameEngine({ mode, onClose, onGameEnd }: GameEngineProps) {
  ...
  const [marcelaMsg, setMarcelaMsg] = useState(
    mode === "ingredients"
      ? MARCELA_MSGS.ingredientStart[0]
      : mode === "order"
      ? MARCELA_MSGS.order[0]
  ...
  const [recipeOptions, setRecipeOptions] = useState<typeof GAME_RECIPES>([]);
  ...
  // Shuffled ingredients for recipe mode
  ...
  // Init recipe options for ingredients mode
  useEffect(() => {
  ...
  // Reset on recipe change (recipe/order modes)
  useEffect(() => {
    if (mode === "order") {
  ...
  // Recipe mode: select all ingredients
        ...
        {feedback && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1.2, opacity: 1 }} exit={{ scale: 1.5, opacity: 0 }}>
              {feedback === "correct" ? <CheckCircle className="w-24 h-24 text-green-500 drop-shadow-lg" /> : <XCircle className="w-24 h-24 text-red-500 drop-shadow-lg" />}
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            <div className="grid grid-cols-2 gap-3">
              {recipeOptions.map((recipe) => (
                ...
                >
                  ...
                  {recipe.country && <span className="text-sm">{recipe.country}</span>}
                </motion.button>
              ))}
            </div>
          </div>
                ...
                {currentRecipe.ingredients.map((ing, idx) => {
                  ...
                  return (
                    <motion.div key={ing} animate={filled ? { scale: [1, 1.2, 1] } : {}}
                      ...
                      {filled ? (
                        <><span className="text-2xl">{ingredient?.emoji}</span><span className="text-[10px] text-primary font-bold mt-0.5 max-w-full px-1 truncate leading-none">{ingredient?.name.split(" ")[0]}</span></>
                      ) : (
